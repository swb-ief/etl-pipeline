...
class ExtractMetricsHistoryTask(luigi.Task):
    ...
    def output(self):
        return dropbox_target(f"/data/history-metrics/{state_districts_hash(self.states_and_districts)}-{self.date}.csv")
...
class CalculateCityMetricsTask(luigi.Task):
    ...
    def run(self):
        ...
        with (
            city_metrics.open('w')
        ) as city_metrics_file, (
            history.open('r')
        ) as history_input_csv, (
            hospitalizations.open('w')
        ) as hosptialization_output:
            calculate_metrics_input(textio2stringio(history_input_csv), start_datetime, tmp_hospitalization.name, city_metrics_file)
