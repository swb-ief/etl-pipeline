...
def extract_wards_data_from_page(positive_cases_page):
    ...
    wards_lines = map(lambda ward_line: ward_line.split(), page_bbox.extract_text().split('\n'))
    result = [ extract_ward_data_from_pdf_line(ward_pdf_line) for ward_pdf_line in wards_lines]
    print(result)
    return result
...
def pdf_data_to_pandas_df(ward_pdf_info):
    ward_positive_df = pd.DataFrame(
        ward_pdf_info, 
        columns=["Ward_Name", "Total_Positive_Cases", "Total_Discharged", "Total_Deaths", "Total_Active"]
        )
