...
def test_delta_calc():
    ...
    df_delta = create_delta_cols(df,['state', 'district', 'ward'], ['total.confirmed','total.recovered','total.deceased','total.active'])

    # check that the result given by create_delta_cols has new cols and values as expected

    assert df_delta is not None
    assert df_delta.at[('MH', 'Mumbai', 'A', '2021-01-01 0:00:00'), 'delta.total.confirmed'] == 0
    assert df_delta.at[('MH', 'Mumbai', 'C', '2021-01-02 0:00:00'), 'delta.total.confirmed'] == 2000
    assert df_delta.at[('MH', 'Mumbai', 'C', '2021-01-03 0:00:00'), 'delta.total.recovered'] == 100
